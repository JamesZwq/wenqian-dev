...
export default function MazePage() {
  ...
  const totalCells = useMemo(() => MAZE_ROWS * MAZE_COLS, []);
  ...
  useEffect(() => {
    const updateCellSize = () => {
      ...
      const verticalReserved = viewportWidth < 768 ? 340 : 240;
      ...
      const nextSize = Math.floor(
        Math.min(maxBoardWidth / MAZE_COLS, maxBoardHeight / MAZE_ROWS)
      );
  ...
  const clearBuildTimers = useCallback(() => {
    ...
    if (buildCleanupTimeoutRef.current) clearTimeout(buildCleanupTimeoutRef.current);
  ...
  useEffect(() => {
    return () => {
      clearBuildTimers();
      if (moveUnlockTimeoutRef.current) clearTimeout(moveUnlockTimeoutRef.current);
  ...
  const resetRuntimeState = useCallback((nextMode: GameMode, withPlayer2: boolean) => {
  ...
  const finalizeBuiltMaze = useCallback((nextMaze: Maze, withPlayer2: boolean) => {
    setMaze(nextMaze);
  ...
  const runBuildAnimation = useCallback(
    (nextMaze: Maze, nextMode: GameMode, withPlayer2: boolean) => {
      ...
      const centerRow = (MAZE_ROWS - 1) / 2;
      const centerCol = (MAZE_COLS - 1) / 2;
    ...
    [clearBuildTimers, finalizeBuiltMaze, resetRuntimeState, totalCells]
  );
  ...
  const {
    ...
    onConnected: ({ direction }) => {
      ...
      if (direction === "outgoing") {
        const nextMaze = generateMaze(MAZE_ROWS, MAZE_COLS);
  ...
  const processNextMove = useCallback(() => {
    ...
    const currentPos =
      nextMove.playerId === 1 ? player1PosRef.current : player2PosRef.current;
    ...
    if (!canMove(currentMaze, currentPos.row, currentPos.col, nextMove.direction)) {
      processNextMoveRef.current();
      return;
    }
    ...
    if (newRow === MAZE_ROWS - 1 && newCol === MAZE_COLS - 1) {
      ...
      const finalElapsed = gameStartTime ? finishedAt - gameStartTime : elapsedTime;
  ...
  }, [elapsedTime, gameStartTime, isConnected, isGenerating, send]);

  useEffect(() => {
    processNextMoveRef.current = processNextMove;
    if (!isGenerating && !isMovingRef.current && inputQueueRef.current.length > 0) {
      processNextMoveRef.current();
    }
  ...
  const queueMove = useCallback(
    (playerId: 1 | 2, direction: Direction) => {
      if (modeRef.current === "menu" || isGenerating || gameEndTimeRef.current) {
        return;
      }
  ...
  const displayMaze = isGenerating ? generationMaze ?? maze : maze ?? generationMaze;
          ...
          {mode !== "menu" && (mode !== "remote" || isConnected) && displayMaze && (
            ...
            >
              ...
              <div className="relative max-w-full overflow-auto border-2 border-[var(--pixel-border)] bg-[var(--pixel-card-bg)] p-2 backdrop-blur-xl md:p-4">
                <svg
                  width={MAZE_COLS * cellSize}
                  height={MAZE_ROWS * cellSize}
                  className="block"
                >
                  ...
                  <AnimatePresence>
                    {(isGenerating || generationMaze) && (
                      ...
                      >
                        ...
                        {displayMaze.map((row, r) =>
                          row.map((cell, c) => {
                            const isRevealed = generationRevealLookup.has(`${r}-${c}`);
                        ...
                        {isGenerating && (
                          <>
                            <motion.rect
                              x={0}
                              y={mazeGenerationProgress * MAZE_ROWS * cellSize - 20}
                              width={MAZE_COLS * cellSize}
                              height={40}
                            ...
                            <motion.line
                              x1={0}
                              x2={MAZE_COLS * cellSize}
                              y1={mazeGenerationProgress * MAZE_ROWS * cellSize}
                              y2={mazeGenerationProgress * MAZE_ROWS * cellSize}
                  ...
                  />
                  <rect
                    x={(MAZE_COLS - 1) * cellSize + 2}
                    y={(MAZE_ROWS - 1) * cellSize + 2}
                    width={cellSize - 4}
                    height={cellSize - 4}
                    fill="var(--pixel-warn)"
                    opacity="0.3"
                  ...
                  {trail.map((t, i) => {
                    ...
                    const color =
                      t.playerId === 1 ? "var(--pixel-accent)" : "var(--pixel-accent-2)";
                    return (
                      <motion.circle
                        key={i}
              ...
              <div className="flex flex-wrap justify-center gap-4 text-xs font-[family-name:var(--font-jetbrains)] text-[var(--pixel-muted)]">
                {mode === "single" && (
                  <div>
                    <span className="text-[var(--pixel-accent)]">P1:</span> WASD
                  </div>
                )}
                {mode === "local" && (
                  <>
                    ...
                    <div>
                      <span className="text-[var(--pixel-accent-2)]">P2:</span> Arrow Keys
                    </div>
                  </>
                )}
                {mode === "remote" && (
                  <div>
                    <span className="text-[var(--pixel-accent-2)]">REMOTE:</span> use WASD or Arrow Keys to move your own avatar
                  </div>
                )}
              </div>
